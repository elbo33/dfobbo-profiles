...
def calculate_algorithm_ratios(min_tau_solved):
    ...
    """
    algo_ratios = {algo: [] for algo in DataProcessor.ALGORITHM_DIRS}
    ...
    for i in range(num_iterations):
        current_values = {
            ...
            for algo in DataProcessor.ALGORITHM_DIRS if i < len(min_tau_solved[algo]) and min_tau_solved[algo][i] != float('inf')
        }

        global_min = min(current_values.values()) if current_values else float('inf')

        for algo in DataProcessor.ALGORITHM_DIRS:
...
def main():
    ...
    DataProcessor.print_formatted_percentages(percentages_below_alpha)
